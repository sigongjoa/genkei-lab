...
2. **분할 규칙 엔진** — 피부 한 부품(목은 몸통) · 이음매는 덮이는 곳으로 · 한 방향으로 빠지나(언더컷)를 Q7 로 → 안 빠지면 쪼갠다.
3. **겹 표현** — 소체 사전(정면 윤곽 · 관절에 맞춤) → 옷 껍질(색 영역 = 겹 후보, 두께는 수치) → 머리카락. 깊이 겹침 손실(천장 +0.057,
   검들기 · 팔앞뒤)도 여기서. 큰 전환이라 예측 먼저 적고 한 장씩.
4. **분할을 재는 자** — 벤치 정답을 실제 키트 분할(가레지킷 분해 사진 · 판매 STL 파츠)로. 지금 Q1~Q6 은 출력되나만 본다.
5. 전에 적은 것 — Q5 떠 있는 부품 잇기 · 사람 아닌 것은 한 부품 + 받침 · MCP 서버와 와이어프레임 화면.
