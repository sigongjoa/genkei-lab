...
돌린 뒤 (09-23 밤, GPT 5장) — B1 틀림 · B2 맞음(겨우):
  F@2mm 중앙 A 0.118 · B 0.118 · C 0.131 · 두 장(정답 옆 그림) 0.275. 정면 한 장은 셋 다 두 장의 절반 아래.
  탐색(예측 밖): 칸마다 정답 D/W 그대로 = 상자 0.250(두 장 0.234 보다 위) · 메카 0.214 · 상자2 0.172, 그러나
  statue 0.026 · 마법사 0.089 — 앞뒤 위치(y)를 0 으로 두고, statue 는 그림 밖으로 잘려 칸이 하나뿐.
  결론: 꼴 라벨보다 **깊이 비율과 앞뒤 위치**가 크다. 등급 셋(0.2/0.6/1.0)은 너무 거칠고(메카 D/W 2~3),
  정면 한 장에는 y 정보가 없다 -> 옆 한 장(생성이든 사람이든)이 꼴 라벨보다 먼저다.
"""
import json
import os
import sys
...
# 돌린 뒤 (09-24) — T1 틀림 · T2 맞음(차 +0.003, 잡음 수준):
#   17장 F 중앙 T0 0.455 · T1 0.443. 장마다 ±: 상자 +0.055 · 상자2 +0.013 · 로봇 +0.024 · 검들기 +0.017,
#   로봇2 -0.037 · 미믹 -0.022 · 드릴 -0.012 · 마법사2 -0.019. 네모 p=8 은 모 깎은 몸통엔 과하다.
#   GPT 5장 T0 0.268 · T2 0.271. 결론: 꼴 라벨은 상자류만 돕고 합은 0 — 안 되는 케이스의 큰 원인이 아니다.
