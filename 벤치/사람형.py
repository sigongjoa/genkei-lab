...
돌린 뒤 (09-22, 46장) — 여덟 중 넷:
  ○ H1 해시 46장 같음.  ✗ H2 B 중앙 0.568 (< 0.60).  ✗ H3 A 0.421 → B 0.568 = +0.147 (< +0.15, 아슬).
  ✗ H4 C 0.745 − B 0.568 = +0.178 (GPT 는 0.05 안이라 했다).  ○ H5 층 타원이 이긴다.
  ○ H6 exe 1순위 중앙 0.745 (지난번 0.35) — **35장 전부 층 타원을 골랐다**.  ○ H7 도형 여섯 그대로.
  ✗ H8 앱 점수 ↔ IoU 순위상관 0.485 (지난번 0.695) — 층 타원이 실루엣을 거의 다 맞춰 앱 점수가 한쪽으로 몰렸다.
  마네킹 A 0.600 · B 0.650 · C 0.870 / 실물 A 0.397 · B 0.498 · C 0.722.
  B 가 지는 곳: 치마 · 망토 · 긴 머리(규칙이 부위를 못 가른다) · 검들기(검이 팔로) · 옆 깊이가 겹친 두께(knight · monster).
  → 숫자는 부위 없는 층 타원, 키트는 부위가 필요하다 — 둘이 갈렸다.
"""
import json
import os
import sys
