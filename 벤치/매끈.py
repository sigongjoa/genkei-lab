...
돌린 뒤 (09-23, 35장) — 셋 중 둘:
  ○ S1 표면각 |우리-정답| 중앙 5.45° → 3.37° (계단이 사라졌다 — 그림 매끈_*.png)
  ✗ S2 F@2mm 0.727 → 0.711 (-0.016) · ○ S3 챔퍼 0.83 → 0.85 mm · 새 각도 0.893 → 0.899
  F 가 떨어진 까닭(추정, 안 쟀다): 단면을 켜 가운데에서만 재 그 사이를 곧게 잇는다 — 치마 끝 · 어깨처럼 폭이 갑자기
  바뀌는 곳에서 모서리를 깎는다. 계단판은 켜 전체에 그 폭을 들고 있었다. 켜를 더 잘게(120) 재면 풀릴지는 따로 시험.
"""
import json
import os
import sys
