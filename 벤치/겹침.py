...
돌린 뒤 (09-24, out/지표_0924b.json) — 둘 다 틀림 -> 혼합 겹침은 뺐다(그림.py 는 층 겹침만):
  떨어졌던 7장 0.806(09-23) · 0.747(층 겹침) · 0.772(혼합 겹침). 35장 0.727 · 0.738 · 0.732.
  혼합 겹침이 잘 되던 사람형(avatarsample_d 0.905 -> 0.899 · e 0.827 -> 0.809 · base 둘 -0.007)까지 가져간다.
  남은 차이는 팔이 아니라 계단(혼합의 켜 로프트) 대 매끈(층쌓기) — 앱 점수(실루엣)는 이 차이를 못 본다.
"""
import json
import os
import sys
