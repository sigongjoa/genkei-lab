"""벤치 시트가 물체를 자른 장을 넓은 창으로 다시 굽는다 (09-24).
  ...
  여기선 실루엣만 굽는다(exe 는 알파만 본다): 창 u ∈ [-0.9, 0.9] · z ∈ [-0.4, 1.4], 1024². 투영은 win.silhouette 과 같다
...
RES, U0, U1, Z0, Z1 = 1024, -0.9, 0.9, -0.4, 1.4
...
def 굽기(case):
    ...
    for 이름, 축 in (("front", 0), ("side", 1)):
        ...
        rgba = np.zeros((RES, RES, 4), np.uint8)
        rgba[m] = (0, 0, 0, 255)
